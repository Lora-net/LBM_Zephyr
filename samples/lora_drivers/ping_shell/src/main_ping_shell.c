...
static int lorademo_ping_send(bool request_not_reply, int counter) {
    ...
    WAIT_FOR(tx_done, PING_TIMEOUT_MS*1000, apps_common_lr11xx_irq_process( context, IRQ_MASK ); k_sleep(K_MSEC(2)));
    if (!tx_done) {
...
static bool rx_replies_done = false;
