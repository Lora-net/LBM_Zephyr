 ...
 * @param pins
 */
static void sx126x_board_event_callback(
	const struct device *dev, struct gpio_callback *cb, uint32_t pins, struct sx126x_hal_context_data_t *data)
{
...
#define DIO2_CONFLICT(node_id) (DT_NODE_HAS_PROP(node_id, dio2_gpios) && DT_NODE_HAS_PROP(node_id, dio2_as_rf_switch)) ||
...
#error Device tree properties dio2-gpios and dio2-as-rf-switch are conflicting, please /delete/ one
#endif

#define DIO3_CONFLICT(node_id) (DT_NODE_HAS_PROP(node_id, dio3_gpios) && DT_NODE_HAS_PROP(node_id, dio3_as_tcxo_control)) ||
...
#error Device tree properties dio3-gpios and dio3-as-tcxo-control are conflicting, please /delete/ one
#endif
...
#define SX126X_DEVICE_INIT(node_id)                                           \
	DEVICE_DT_DEFINE(node_id, sx126x_init, PM_DEVICE_DT_GET(node_id),         \
			&sx126x_data_##node_id, &sx126x_config_##node_id,                 \
