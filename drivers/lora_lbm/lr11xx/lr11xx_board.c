...
static int lr11xx_init(const struct device *dev)
{
	...
	k_thread_create(&data->thread, data->thread_stack, CONFIG_LORA_BASICS_MODEM_DRIVERS_EVENT_TRIGGER_THREAD_STACK_SIZE,
...
#define DT_PROP_BY_IDX_U8(node_id, prop, idx) \
...
#define LR11XX_CHIP_TYPE(node_id) \
	COND_CODE_1(DT_NODE_HAS_COMPAT(node_id, semtech_lr1110), (LR11XX_SYSTEM_VERSION_TYPE_LR1110), \
	(COND_CODE_1(DT_NODE_HAS_COMPAT(node_id, semtech_lr1120), (LR11XX_SYSTEM_VERSION_TYPE_LR1120), \
	(COND_CODE_1(DT_NODE_HAS_COMPAT(node_id, semtech_lr1121), (LR11XX_SYSTEM_VERSION_TYPE_LR1121), \
		(/**/) \
	))))) \
...
#define LR11XX_DEVICE_INIT(node_id)                                           \
	DEVICE_DT_DEFINE(node_id, lr11xx_init, PM_DEVICE_DT_GET(node_id),         \
			&lr11xx_data_##node_id, &lr11xx_config_##node_id,                 \
...
#define LR11XX_DEFINE(node_id)                                                                       \
	...
	static const struct lr11xx_hal_context_cfg_t lr11xx_config_##node_id = LR11XX_CONFIG(node_id);   \
